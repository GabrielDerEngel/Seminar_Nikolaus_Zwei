...
namespace LValueRValue {
    ...
    // lvalue reference / eine Referenz eines Objekts MIT Namen
    void sayHello(std::string& message) {
        std::cout << "sayHello [std::string&]:  " << message << std::endl;
    }

    // rvalue reference / eine Referenz eines Objekts OHNE Namen
    void sayHello(std::string&& message) {
        std::cout << "sayHello [std::string&&]: " << message << std::endl;
    }

    void helper(std::string&& message) {  // rvalue reference 
    ...
    void test01() {
        ...
        sayHello(a);
        sayHello("ABC");
        sayHello(a + b);  // a + b = tmp  / anonymes Objekt
    }
