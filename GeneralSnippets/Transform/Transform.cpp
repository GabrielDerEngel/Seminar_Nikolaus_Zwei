...
namespace AlgorithmTransform {
    ...
    void test_01()
    {
        ...
        std::vector<std::string> names;  // set size of vector (!)

        // std::transform on a single range - retrieve names from phonebook
        std::transform(
            std::begin(phonebook),
            std::end(phonebook),
            std::back_inserter(names),   // push_back an names i.g. zu names[i]
            //std::begin(names),    // beginning of the destination range - DER BEREICH MUSS EXISTIEREN
