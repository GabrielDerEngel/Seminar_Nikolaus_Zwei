...
namespace TypeTraits_Demo_Iterator
{
    ...
    template <typename T>
    using ValueType2 = std::vector<T>::value_type;

    void test() {

        std::vector<long> vec2;
        std::vector<short> vec3;

     //   std::remove_reference<decltype (vec2[0])>::type

        // std::remove_reference<decltype (vec3[0])>::type


        if (std::is_same <
            std::remove_reference<decltype (vec2[0])>::type,
            std::remove_reference<decltype (vec3[0])>::type>::value) {
            std::cout << "...";
        }
        else
        {
        ...
        if (std::is_same <std::vector<long>::value_type, std::vector<int>::value_type>::value) {
            std::cout << "...";
        }

     //  std::vector<long>::value_type wert2;
     //
